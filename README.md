...
# verify that ecDNAInspector has been installed correctly by running:
ecI --help
# if installation was successful, the available command-line options will be displayed.
```
...
Input paths should include your cycle files (AmpliconArchitect output or manually formatted processed cycle files), amplicon_info.csv, SV file, blacklist file, gene file, and BED files (if available). If starting from AmpliconArchitect output, the processed_cycle_files directory should initially be empty and ecDNAInspector will fill it automatically. Otherwise, manually formatted cycle files should be deposited into the processed_cycle_files directory. Other intermediate and output files will be generated throughout the pipeline.

Start by running "ecDNAInspector_cycle_selection.ipynb", then "ecDNAInspector_analysis.ipynb". Run "ecDNAInspector_Jaccard_calculations.ipynb" before the analysis notebook if you wish to make structural conservation comparisons (note that large datasets may require substantially longer to process, as this step compares each cycle to every other cycle.). Please see the manuscript for more details on the different notebooks and their functionalities.

### Command Line Version

Once you are comfortable with your selected parameters, you can run ecI in a more streamlined fashion using the command line interface version. Note that the command line version is required to run ecI with HPC clusters, which may be necessary for large datasets. ecI is written as a package so you can import it to any code you like, or run directly from the command line. 
...
| processed_cycle_files    | path to directory for processed cycle files  | **Required.** Please make sure this directory is established before running the cycle metric calculations. If starting from AA output, create this directory and leave it empty. If not using AA for initial cycle prediction, please store your cycle predictions here in the default format. See provided test processed cycle files for required format. |  |
| bed_files_path    | path to directory with .bed coverage files for samples | **Optional, but required for pFNR calculation.** If BED coverage files are unavailable, pFNR cannot be calculated. If so, pFNR must be removed from all downstream analyses, including feature lists, confidence assignment, and any references to Cycle PFNR, cycle_PFNR, and PFN_list, in addition to setting weights = "PFNR": 0. |   |
...
| **Output files: specify where ecI outputs tables and figures**    |   |   |   |
| unfiltered_cluster_image_output    | path to file where unfiltered clustering image will be stored, needs to be defined  | **Required.**  |   |
| intrasamp_filtered_cluster_image_output    | path to file where intrasample-filtered clustering image will be stored, needs to be defined  | **Required.**  |   |
...
| **Structural validation parameters**    |   |  |  |
...
| **Confidence assignment parameters**    |   |  |  |
